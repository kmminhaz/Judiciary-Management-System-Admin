        ...
        <div class="mt-5 pt-5 col-12 col-md-9 col-lg-7 col-xl-6">
          <div class="card shadow-lg" style="border-radius: 15px;">
            <div class="card-body p-5">
              <h4 class="text-uppercase text-center mb-5 fw-bold">Manage Law's</h4>

            <?php
                $linkId = $_GET['id'];
                $linkTarget = $_GET['target'];
              
                $sql = "SELECT * FROM `laws_link` WHERE `id` = '$linkId'";
                ...
                <div class="d-flex justify-content-between">
                    <?php
                        if($linkTarget == 'add'){
                            ...
                            ?>
                                <button type="submit" name="delete" class="btn btn-danger btn-block btn-lg text-white m-2 mt-3" onclick="return confirm('Are you sure, You want to delete this Law !!');">Delete</button>
...
if(isset($_POST['add'])){
    ...
    $sqladd = "INSERT INTO `laws_link` (`title`, `link`) 
      VALUES ('$title', '$link')";
    $res = mysqli_query($conn, $sqladd) or die(mysqli_connect_error());

    if($res){

      $_SESSION['add'] = "<div class='text-success text-center'> <strong>A new Law has been added</strong> </div>";
    ?>
        <script type="text/javascript">
            window.location.href = '<?php echo SITEURL; ?>laws.php';
        </script>
    <?php
    }else{
...
if(isset($_POST['update'])){
    ...
    if($res){
      ...
      $_SESSION['update'] = "<div class='text-success text-center'> <strong>The Law has been updated</strong> </div>";
    ?>
        <script type="text/javascript">
            window.location.href = '<?php echo SITEURL; ?>laws.php';
        </script>
    <?php
      ...
      $_SESSION['update'] = "<div class='text-danger text-center'> <strong>The Law are not updated</strong> </div>";
    ?>
        <script type="text/javascript">
            window.location.href = '<?php echo SITEURL; ?>laws.php';
        </script>
...
<?php
    if(isset($_POST['delete'])){
        $deleteQuery = "DELETE FROM `laws_link` WHERE id = '$linkId'";
        
        $resDelete = mysqli_query($conn, $deleteQuery);

        if($resDelete){
            $_SESSION['delete'] = "<div class='text-success text-center py-3'><strong?> The Law is Deleted Successfully </strong></div>";
            ?>
            <script type="text/javascript">
                window.location.href = '<?php echo SITEURL; ?>laws.php';
            </script>
        <?php
            ...
            $_SESSION['delete'] = "<div class='text-danger text-center py-3'><strong?> The Law is Deletation Failed </strong></div>";
            ?>
            <script type="text/javascript">
                window.location.href = '<?php echo SITEURL; ?>laws.php';
            </script>
